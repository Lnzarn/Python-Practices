...
def check_details(connection, user_id):
    query = "SELECT balance, name, age, email FROM users WHERE id = ?"
    try:
        with connection:
        ...
        print(f"Your Balance is {balance:.2f} PHP")
    except Exception as e:
        print("Error:", e)


def deposit_money(connection, user_id):
